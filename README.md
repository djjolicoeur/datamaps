 ...
 * Does not support the Pull api currently
 * Need to figure out if I can swap add the ability to choose query engine provider
   so people can use datascripts engine, should they choose.  I use datomic in my
   projects, so I went with that first. 
...
manipulate and query data, this will do the trick. The facts generated by datamaps are not
intended to be added to or retracted from so datamaps is very much _not_ a database solution. 
